...
collapse (sum) _d fu, by(agec cal wk)

* direct standardization
qui:dstdize _d fu agec cal, by(wk) using(data/stdpop2)
  matrix ir = r(adj)
  matrix ub = r(ub) 
  matrix lb = r(lb)
...
* extract SIR     
 ...
 foreach k of local lvl { 
   ...
   matrix A[`k',2] = `j'
   matrix A[`k',3] = `k'
   matrix A[`k',4] = ir[1,`k']*1000*365.25
   matrix A[`k',5] = lb[1,`k']*1000*365.25
   matrix A[`k',6] = ub[1,`k']*1000*365.25
 } 
